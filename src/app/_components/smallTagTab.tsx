...
export default function SmallTagTab({
  tags,
  token,
}: {
  tags: Tag[];
    ...
    <div className="text-sm flex flex-col gap-3">
      <div className="flex items-center w-full">
        ...
        <button
          ...
          } gap-1 px-3 py-2 text-[12px] transition-all active:scale-98 no-select duration-200 ease-in-out hover:bg-gray-500 hover:scale-105 hover:shadow-lg bg-gray-600 rounded-2xl text-white`}
        >
          <FaPlus size={12} />
          <span>New Tag</span>
        </button>
      ...
      <ul className="mb-4">
        {tags.map((tag) => (
          <li
            key={tag.id}
            className="mb-3 border border-light-gray p-2 rounded-lg text-sm flex items-center justify-between"
          >
